 ...
 //var  x1=1;
	...
	$(add_button).click(function(e){
		...
		if(x < max_fields){ //max input box allowed
			...
			$(wrapper).append('<div class="abc"> <div class="row"> <div class="col-md-3"> <div class="form-group"><label>Variation Group Name</label><input type="text" name="group_name['+this.x+'][name]" class="form-control"></div> </div>  <div class="col-md-1"> <div class="form-group actions"><button type="button" class="btn btn-danger waves-effect waves-light"><a href="#" class="remove_field"><i class="fa fa-close"></i>Remove</a></button></div> </div> </div> <div class="fields"><div class="row"> <div class="col-md-3"> <div class="form-group"><label>Options</label><input type="text" name="group_name['+this.x+']['+this.x1+'][options]" class="form-control"> <input type="hidden" value="1" name="count[]" ></div> </div> <div class="col-md-3"> <div class="form-group"><label>Regular Price</label><input type="number" name="group_name['+this.x+']['+this.x1+'][reg_price]" class="form-control"> </div> </div> <div class="col-md-3"> <div class="form-group"><label>Sale Price</label><input type="number" name="group_name['+this.x+']['+this.x1+'][off_price]" class="form-control"></div> </div> <div class="col-md-3"> <div class="form-group actions"><button type="button" class="btn btn-danger waves-effect waves-light"><i class="fa fa-close"></i>Remove</button> <button type="button" class="btn btn-success"><a href="#" class="add_field"><i class="fa fa-plus"></i>Add</a></button></div></div> </div> </div> </div> </div>'); //add input box
		}
	});
	$(wrapper).on("click",".remove_field", function(e){ //user click on remove text
		e.preventDefault();
                $(this).parents('div.abc').remove(); this.x--;
              delete x1;
	});
	$(wrapper).on("click",".remove_field1", function(e){ //user click on remove text
          //  alert('hii');
		e.preventDefault();
                $(this).parents('.fields .row').remove(); this.x1--;
             // delete x1;
	});
         // var wrapper   		= $(".variations"); //Fields wrapper
        ...
        $(wrapper).on("click",".add_field", function(e){ //user click on remove text
         //  console.log(x1);
            ...
            e.preventDefault();
            if(x1 < max_fields2){ //max input box allowed
			this.x1++;
                       // alert(x);
                         $(this).parents('.fields').append(' <div class="row"> <div class="col-md-3"> <div class="form-group"><label>Options</label><input type="text" name="group_name['+this.x+']['+this.x1+'][options]" class="form-control"> <input type="hidden" value="1" name="count[]" ></div> </div> <div class="col-md-3"> <div class="form-group"><label>Regular Price</label><input type="number" name="group_name['+this.x+']['+this.x1+'][reg_price]" class="form-control"> </div> </div> <div class="col-md-3"> <div class="form-group"><label>Sale Price</label><input type="number" name="group_name['+this.x+']['+this.x1+'][off_price]" class="form-control"></div> </div> <div class="col-md-3"> <div class="form-group actions"><button type="button" class="btn btn-danger "><a href="#" class="remove_field1"><i class="fa fa-close"></i>Remove</a></button> <button type="button" class="btn btn-success"><a href="#" class="add_field"><i class="fa fa-plus"></i>Add</a></button></div> </div></div>');
                    }
...
$(this).parent().find(".ion-minus-circled").removeClass("ion-minus-circled").addClass("ion-plus-circled");
 ...
 setInterval(function(){    
    ...
    success : function(response){
        ...
        {
             $("#noti").append("<a href='<?php echo base_url();?>seller/order?driver_status=1' class='dropdown-item notify-item'><div class='notify-icon bg-primary'><i class='mdi mdi-cart-outline'></i></div><p class='notify-details'> Order has been accepted<span class='text-muted'>You have "+response.accept+" accepted orders</span></p></a>"); count++;
        }
         if(response.ready!=0)
        {
